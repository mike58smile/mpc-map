...
% C observes x/y only; heading is inferred from motion and occasional GNSS
% displacement, because GNSS does not directly measure orientation.
...
if isfield(read_only_vars, 'gnss_position') && numel(read_only_vars.gnss_position) >= 2 && ...
	...
	% If GNSS is available on the first frame, start the EKF there.
	mu_xy = read_only_vars.gnss_position(1:2);
elseif isfield(public_vars, 'particles') && ~isempty(public_vars.particles)
	% In GNSS-denied starts, initialize near the particle cloud instead of an
	% arbitrary map coordinate.
...
% Heading variance is intentionally large: initial orientation is unknown
% unless the particle filter or later GNSS displacement constrains it.
